...
const AgentDashboard: React.FC<CommandCenterProps> = ({ 
  properties, 
  onEdit, 
  onDelete, 
  onAddProperty, 
  ...
  useEffect(() => {
    const filters = { agentId: agentProfile.id };
    ...
    const unsubscribeActivities = subscribeToActivities({ userId: agentProfile.id }, (data) => {
      setActivities(data);
    });
  ...
  const handleCreateLead = async (data: any) => {
    ...
    try {
      ...
      const id = await createLead(leadData, agentProfile.id, null);
      await logActivity({
           ...
           <div className="flex items-center gap-4">
             ...
             <div className="space-y-1.5 md:space-y-2">
               ...
               <p className="text-[8px] md:text-[10px] font-black text-navy-muted uppercase tracking-[0.3em] md:tracking-[0.4em] flex items-center gap-2">
                 {agentProfile.agency ? (
                   <><i className="fa-solid fa-building-shield text-gold"></i> {agentProfile.agency}</>
                 ) : (
                   <><i className="fa-solid fa-building-shield text-gold"></i> Independent Agent</>
                 )}
               </p>
             </div>
           </div>
