...
const COLUMNS: LeadStatus[] = ['New', 'pending', 'calling', 'Contacted', 'Site Visit', 'Negotiation', 'Closed', 'Lost'];
...
const LeadPipeline: React.FC<LeadPipelineProps> = ({ leads, onLeadClick, onStatusChange, onAddLead }) => {
  ...
  const getStatusDisplayName = (status: string) => {
    if (status === 'pending') return 'AI Queue';
    if (status === 'calling') return 'AI Calling';
    return status;
  };
      ...
      {COLUMNS.map((status, idx) => (
        ...
        >
          <div className="flex items-center justify-between px-2">
            <h3 className="text-[10px] font-bold uppercase tracking-widest text-navy flex items-center gap-2">
              <span className={`w-2 h-2 rounded-full ${
                status === 'New' ? 'bg-blue-500' : 
                status === 'pending' ? 'bg-purple-500' :
                status === 'calling' ? 'bg-pink-500 animate-pulse' :
          ...
          <div className="flex-1 bg-beige-100/30 rounded-2xl p-2 space-y-3 border border-dashed border-beige-200 min-h-[400px] transition-colors hover:bg-beige-100/50">
            ...
            {leads.filter(l => l.status === status).length === 0 && (
              <div className="h-32 flex flex-col items-center justify-center text-[8px] font-bold uppercase tracking-widest text-navy-muted/20 border-2 border-dashed border-beige-200/50 rounded-xl">
