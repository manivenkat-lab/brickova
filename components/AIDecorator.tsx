...
const STYLES = [
  { id: 'modern', label: 'Mid-Century Modern', icon: Sofa },
  { id: 'minimal', label: 'Japandi Minimalism', icon: Leaf },
  { id: 'industrial', label: 'Industrial Loft', icon: Building },
  { id: 'scandinavian', label: 'Nordic Clean', icon: Snowflake },
  { id: 'luxury', label: 'European Baroque', icon: Crown }
];
      ...
      <div className="bg-beige-50 w-full max-w-6xl rounded-[3rem] overflow-hidden shadow-premium flex flex-col md:flex-row h-[85vh] border border-beige-300">
        <div className="w-full md:w-80 bg-white border-r border-beige-200 p-10 flex flex-col space-y-10">
          ...
          <div className="space-y-4">
            <h4 className="text-[10px] font-bold text-navy uppercase tracking-widest">Select Canvas</h4>
          ...
          <div className="space-y-4 flex-1">
            <h4 className="text-[10px] font-bold text-navy uppercase tracking-widest">Style Palette</h4>
            <div className="space-y-2">
              {STYLES.map(style => (
                <button key={style.id} onClick={() => setSelectedStyle(style.id)} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-[10px] font-bold uppercase tracking-widest transition-all ${selectedStyle === style.id ? 'bg-navy text-white shadow-soft' : 'bg-beige-100 text-navy-muted hover:text-navy'}`}>
                  <style.icon className="w-4 h-4 opacity-60" />
                  {style.label}
                </button>
              ))}
            </div>
          ...
          <button onClick={handleRestyle} disabled={isProcessing} className="w-full bg-gold text-white py-5 rounded-2xl font-bold uppercase text-[10px] tracking-widest shadow-premium hover:bg-gold-dark transition-all disabled:opacity-50 flex items-center justify-center gap-3">
        ...
        <div className="flex-1 bg-beige-100 relative overflow-hidden flex flex-col">
          <div className="h-16 flex items-center justify-between px-10 border-b border-beige-200 bg-white/50 backdrop-blur-md">
            <span className="text-[9px] font-bold uppercase text-navy-muted tracking-wider">Rendering Engine: V3.1 Neural</span>
            <button onClick={onClose} className="text-navy hover:text-gold transition-all"><X className="w-6 h-6" /></button>
          </div>
             ...
             <div className="relative w-full max-w-4xl aspect-[16/9] bg-white rounded-[2.5rem] shadow-premium overflow-hidden border border-beige-200">
               {isProcessing ? (
                 <div className="absolute inset-0 z-20 bg-white/60 backdrop-blur-md flex flex-col items-center justify-center text-center space-y-6">
                    ...
                    <p className="text-xs font-bold uppercase text-navy tracking-wider">Synthesizing Interior Architecture...</p>
                 </div>
               ) : null}
               ...
               {resultImage && !isProcessing && (
                 <div className="absolute bottom-10 left-10 right-10 flex justify-between items-center bg-navy/80 backdrop-blur-xl p-6 rounded-3xl border border-white/10">
                    <div className="text-white">
                       <p className="text-[8px] font-bold uppercase tracking-widest opacity-60 mb-1">Generated Output</p>
                       <h5 className="text-xs font-bold uppercase">{selectedStyle} Protocol Applied</h5>
                    </div>
                    <button onClick={() => setResultImage(null)} className="text-[10px] font-bold uppercase text-white border border-white/20 px-6 py-2 rounded-xl hover:bg-white/10">Reset Protocol</button>
                 </div>
               )}
             </div>
          </div>
