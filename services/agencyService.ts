...
export const createAgency = async (formData: { name: string, adminUid: string }) => {
  ...
  try {
    ...
    handleFirestoreError(error, OperationType.WRITE, "agencies");
    throw error;
  }
};
  ...
  try {
    ...
    const memberPromises = snapshot.docs.map(async (memberDoc) => {
      ...
      try {
        ...
        handleFirestoreError(error, OperationType.GET, `users/${memberData.uid}`);
        return { ...memberData, userDetails: null };
      }
    });

    return await Promise.all(memberPromises);
  } catch (error) {
    handleFirestoreError(error, OperationType.GET, `agencies/${agencyId}/members`);
    return [];
  }
};
